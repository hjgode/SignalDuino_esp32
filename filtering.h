...
/*
struct Puffer {
  int Value;   // Kann auch byte etc sein. Bin mir aber nicht sicher ob wir so einen Array anlegen k�nnen
} ;
*/
...
class RingBuffer {
  ...
  void addValue(int *M);
	void resetBuffer();
	void resetFReadPointer();       // Funktion liefert das N�chste Element des feien Lesezeigers und verschiebt diesen
...
/*
  **Experimentell**
  ...
  den gleichen Puffer nutzen (*head und *end sind statisch �ber alle Instanzen). Dadurch k�nnen mehrere
...
class MultiRing : public RingBuffer {
	public:
		...
		MultiRing *next_obj; // Zeiger auf N�chstes Objekt
